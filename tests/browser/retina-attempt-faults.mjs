...
try {
  ...
  const results=[];
  for(const mode of ["context","unresponsive"]) results.push(await page.evaluate(async ({root,mode})=>{
    ...
    return await new Promise((resolve,reject)=>{
      ...
      worker.onmessage=({data})=>{
        ...
        if(reply.type==="frames") {
          if(data.generation===1) return finish(reject,Error("Failed acquisition published a frame"));
          frames++;
        }
        if(reply.type==="error") {
          if(data.generation!==1) return finish(reject,Error(reply.message));
  ...
  for(const result of results) {assert.ok(result.failure.settledMs<5000);assert.equal(result.retryFrames,1);}
