...
const output = process.env.CAMERA_OUTPUT ?? new URL("../../specs/help-the-fly-escape/assets/evidence/06/", import.meta.url).pathname;
...
const follow = async (id) => page.waitForFunction(id => {
...
const released = () => page.waitForFunction(() => JSON.parse(document.querySelector('[data-testid="playback-report"]').textContent).camera?.following === false);
...
try {
  ...
  const card = await lastCard.boundingBox(), roster = await page.getByLabel("Fly roster").boundingBox();
  assert.ok(card.y >= roster.y && card.y + card.height <= roster.y + roster.height, "Low card is fully visible");
  ...
  assert.equal(await page.getByRole("button", {name: `Select fly ${picked + 1}`, exact:true}).getAttribute("aria-pressed"), "true");
  await page.getByRole("button", { name: "Overview", exact: true }).click();
  await released();
  ...
  assert.equal(await page.getByTestId("selected-fly").getAttribute("data-fly-id"), String(otherPicked));
  await lastCard.click();
  await follow(19);
  ...
  await writeFile(`${output}/checks.json`, JSON.stringify({ initial: initial.camera, selected: selected.camera, close: close.camera, overview: overview.camera, errors }, null, 2));
