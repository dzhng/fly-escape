...
The [banana](banana/README.md) shares the apple’s native edible-surface and placement ownership. An edible surface classification does not establish a successful placement strategy. Timed campaign rounds retain odor and contact without feeding or life extension.

Diagnostic scent crumbs are odor-only floor cues, absent from campaign inventory. Their Blender source in [author.py](author.py) has a radius-one footprint and shallow relief; the placement renderer scales X/Z using the core catalog. This contract keeps a scent marker from implying an unmodeled solid obstacle.
...
[Seam regression evidence](../../specs/done/help-the-fly-escape/assets/evidence/20/mesh-seams/README.md)
