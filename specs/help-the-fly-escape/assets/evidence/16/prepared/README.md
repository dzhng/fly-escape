...
The read-only greybox.html/greybox.ts entry imports the shared WorldView and authored asset loaders, consumes candidate Geometry/placements, and places one static diagnostic fly at the fork. It has no Worker, second layout or movement controller. catalog.json is a captured output of Rust tool_catalog from the native test, used only by this evidence harness. The browser captures show Overview, the reference junction, and the same junction with poor fan directions. browser.json records zero browser errors after a harmless initial favicon 404 was fixed in the harness. Native and browser evidence do not establish neural outcomes.

Reproduce with the repository Vite server rooted at the checkout on port 5278 and open this HTML path. capture.mjs drives the visible controls and saves the three full frames. Run `cargo test -p sim --test level2_prepared -- --nocapture` for geometry/field/placement checks; the printed catalog is the snapshot source. The standard app and house workbench fixture are untouched.
