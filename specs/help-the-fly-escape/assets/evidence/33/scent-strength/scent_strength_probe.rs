...
/// Level with the placed banana's emission scaled. The banana itself is placed
/// through the shared tool catalog at its authored rate 1; a co-located source of
/// the same radius supplies the surplus. Injection is linear in rate and both share
/// one footprint, so this is exactly a rate-`multiplier` banana. Fixed household
/// sources, geometry, exit suction and the (empty) fan list are untouched.
fn scaled_level(content: &Content, banana: &Placement, multiplier: f64) -> LevelDef {
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if args.len() != 7 {
        return Err("Usage: scent_strength_probe GRAPH_DIR CONTENT_JSON OUTPUT_JSON DURATION_TICKS binary|saturating SEED_INDEX".into());
    }
    ...
    // The only content edits: the diagnostic horizon and the scratch encoding knob.
    ...
    let identity = json!({
        ...
        "probeSourceHash": format!("{:x}", Sha256::digest(include_bytes!("scent_strength_probe.rs"))),
        ...
        "unchanged": "fixed household sources, geometry, exit suction, cue gains, thresholds, neural dynamics, spawn; no fan is placed in any arm",
    });

    let mut arms = vec![];
    for (label, multiplier, placed) in [
        ("empty", 1.0, false),
        ("banana1x", 1.0, true),
        ("banana3x", 3.0, true),
    ] {
        let level = scaled_level(&content, &banana, if placed { multiplier } else { 1.0 });
