...
Shape/diff/docs review is clean. The ring remains renderer-owned and uses the existing camera projection; no second camera math, fixture correction, extra marker, shader or resource lifetime was introduced. The production change is 25 added lines net. The pre-implementation [marker decision](../../../slices/06-camera-selection.md#marker-readability-decision) records the material choice. The choice to thicken inward keeps one-fly association; retaining a central hole keeps the cue a ring. Reusing the existing mesh limits GPU work to its existing vertex buffer.
