...
try {
  ...
  await page.addInitScript(() => {
    window.startupTrace = []; window.recordHashes=[];
      ...
      set onmessage(receive) {
        super.onmessage = event => {
          ...
          if (reply?.type === 'frames') {
            const hashes={tick:reply.chunk.startTick};
            for(const [key,value] of Object.entries(reply.chunk)) if(ArrayBuffer.isView(value)) {
              const bytes=new Uint8Array(value.buffer,value.byteOffset,value.byteLength); let hash=2166136261;
              for(const byte of bytes) hash=Math.imul(hash^byte,16777619)>>>0;
              hashes[key]=[bytes.length,hash];
            }
            window.recordHashes.push(hashes);
          }
  ...
  for (let run = 0; run < runs; run++) {
    ...
    await writeFile(`${output}/hashes.json`, JSON.stringify(await page.evaluate(()=>window.recordHashes)));
