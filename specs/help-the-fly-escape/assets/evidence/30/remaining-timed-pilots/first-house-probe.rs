...
//!
//! Usage: placement_pilot GRAPH_DIR PLANS_JSON SEEDS_CSV OUTPUT_DIR [--validate-only]
//! Reports carry identities, inputs and per-fly aggregates only; per-tick event and
//! frame logs are deliberately omitted.
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if args.len() < 5 {
        return Err("Usage: placement_pilot GRAPH_DIR PLANS_JSON SEEDS_CSV OUTPUT_DIR [--validate-only]".into());
    }
    let validate_only = args.get(5).map(String::as_str) == Some("--validate-only");
    ...
    for plan in &plans {
        let resolved = sim::placement::resolve_placements(&level, &plan.placements)
            .map_err(|e| format!("plan {} rejected by resolve_placements: {e}", plan.label))?;
        if validate_only {
            println!(
                "plan {} accepted: placements={} sources={} fans={}",
                plan.label,
                serde_json::to_string(&resolved.state.placements)?,
                serde_json::to_string(&resolved.sources)?,
                serde_json::to_string(&resolved.field_config.fans)?,
            );
        }
    }
    if validate_only {
        ...
        for plan in &plans {
            ...
            let result = loop {
                ...
                for fly in &frame.flies {
                    ...
                    // Aggregate counts only; per-tick event and frame logs are omitted.
                    for event in &fly.events {
                        match event.kind {
                            sim::body::BodyEventKind::FeedingStarted => trace.feeding_bouts += 1,
                            sim::body::BodyEventKind::ModeChanged { .. } => mode_changes += 1,
            ...
            let flies: Vec<_> = traces
                ...
                .map(|(id, t)| {
                    json!({"id":id,"outcome":t.outcome,"terminalTick":t.terminal_tick,
                        ...
                        "flyingTicks":t.flying_ticks,"contactTicks":t.contact_ticks,
                        "roomTicks":t.rooms,"offRoomTicks":t.off_room_ticks,
