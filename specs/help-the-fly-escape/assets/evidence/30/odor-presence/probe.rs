//! Scratch pilot for the bounded odor-presence sensory candidate. Byte-identical in
//! measurement logic to /tmp/second-house-confirm/source/second_house_pilot.rs; the only
//! additions are (a) a sensory-encoder preflight that records the candidate's cue_currents
//! truth table before any attempt runs, and (b) a sensorySourceHash in each report. No
//! motion, steering or policy shortcuts; plans are validated by the production placement
//! owner before any attempt is constructed. `speedScale` scales only bodyConfig
//! walkSpeed/flightSpeed; this run uses 1.0 because the level is already paced.
//!
//! Usage: odor_presence_pilot GRAPH_DIR PLANS_JSON SEEDS_CSV OUTPUT_DIR
...
use sim::{
    ...
    environment::{FieldSample, Point, SensorySample},
    placement::Placement,
...
/// Preflight over the real `cue_currents` on the shipped graph: records what the candidate
/// encoder emits for the cases the review must check, so the claims in the report are read
/// off the same function the attempts use rather than restated from the source.
fn encoder_table(graph: &Graph) -> Result<Value, Box<dyn std::error::Error>> {
    let readouts = sim::sensory::motor_readout_indices(graph);
    let mut rows = Vec::new();
    // (label, left magnitude, right magnitude) applied to whichever pathway is under test.
    let cases = [
        ("absent", 0.0, 0.0),
        ("sub-threshold", 0.049, 0.0),
        ("sub-threshold-bilateral", 0.04, 0.04),
        ("at-threshold-bilateral", 0.05, 0.05),
        ("present-bilateral-equal", 0.8, 0.8),
        ("present-left-stronger", 0.8, 0.2),
        ("present-right-stronger", 0.2, 0.8),
        ("present-anatomical-left", 0.5002, 0.5),
    ];
    for (pathway, name) in [
        (CuePathway::ExcitatoryOdor, "excitatoryOdor"),
        (CuePathway::InhibitoryOdor, "inhibitoryOdor"),
        (CuePathway::Vision, "vision"),
    ] {
        for (label, left, right) in cases {
            let field = |v: f64| match pathway {
                CuePathway::ExcitatoryOdor => FieldSample {
                    repellent_odor: v,
                    ..Default::default()
                },
                CuePathway::InhibitoryOdor => FieldSample {
                    attractive_odor: v,
                    ..Default::default()
                },
                _ => FieldSample {
            ...
            // Campaign gain for the odor pathways on this level is 2.
            let currents = cue_currents(graph, &sample, pathway, 2.)?;
            let groups: BTreeMap<String, Vec<f64>> = graph
                .manifest
                .groups
                .iter()
                .filter(|g| {
                    matches!(
                        g.id.as_str(),
                        "odorExcL" | "odorExcR" | "odorInhL" | "odorInhR" | "visionL" | "visionR"
                    )
                })
                .map(|g| {
                    let mut seen: Vec<f64> = g
                        .indices
                        .iter()
                        .filter_map(|i| {
                            currents
                ...
                .collect();
            let mut distinct: Vec<String> = currents.iter().map(|(_, c)| format!("{c}")).collect();
            distinct.sort();
            distinct.dedup();
            rows.push(json!({
                "pathway": name,
                ...
                "leftInput": left,
                "rightInput": right,
                "nonzeroCount": currents.iter().filter(|(_, v)| *v != 0.).count(),
                "totalCurrent": currents.iter().map(|(_, v)| v).sum::<f64>(),
                "distinctValues": distinct,
                "perGroupDistinctCurrents": groups,
                "touchesMotorReadout": currents.iter().any(|(i, v)| *v != 0. && readouts.contains(i)),
                "anyNegative": currents.iter().any(|(_, v)| *v < 0.),
            }));
        }
    }
    Ok(json!({
        ...
        "note": "Read directly off production cue_currents with the candidate applied. \
Odor group current = gain * encoded side value; vision rows are the unchanged control.",
        "rows": rows,
    }))
}
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if args.len() != 5 {
        return Err("Usage: odor_presence_pilot GRAPH_DIR PLANS_JSON SEEDS_CSV OUTPUT_DIR".into());
    }
    ...
    drop(bytes);
    let encoder = encoder_table(&graph)?;
    std::fs::write(
        format!("{}/encoder-verification-{}.json", args[4], args[3].replace(',', "_")),
        serde_json::to_string_pretty(&encoder)?,
    )?;
    eprintln!("encoder verification written");
    ...
    for &seed in &seeds {
        for plan in &plans {
            ...
            let report = json!({
                ...
                "pilotSourceHash":format!("{:x}",Sha256::digest(include_bytes!("odor_presence_pilot.rs"))),
                "sensorySourceHash":format!("{:x}",Sha256::digest(include_bytes!("../src/sensory.rs"))),
