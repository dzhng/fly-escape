...
The right olfactory descending-neuron list contained two annotation-classified central-brain intrinsic cells: body13500 (LHAV2p1) and body12212 (PVLP143). Both have been dropped from OLFACTORY_DN_RIGHT, which now holds twenty-three candidates, and the exporter refuses any readout candidate that the source does not annotate as a descending neuron on the named soma side. Body13500 stays an excitatory smell candidate and, no longer being a readout, is no longer withheld by the motor-readout guard, so excitatory injection now reaches ten left and ten right cells. Removing the guard itself would directly stimulate a measured output and remains unacceptable. Interpreting a new sensory experiment still requires the anatomical scope of the retained circuit, not compensation for an assumed direction of behavior.
