...
The player shapes the environment before releasing the flies. Each level supplies a fixed inventory, without money or a shop. Attempts use fresh random seeds; replay preserves the same attempt. One star unlocks the next level; higher stars reward more escaped flies. Attempts use a fixed time budget rather than starvation. Feeding and life extension are deferred for MVP; food still provides odor and physical landing/walking surfaces that can distract and slow flies. Only flies that physically escape count toward the score. Object effects are probabilistic: useful placements should improve the average number of escapes across twenty-fly swarms, not guarantee a response from every fly. Judge campaign effectiveness with matched-seed swarm comparisons.
