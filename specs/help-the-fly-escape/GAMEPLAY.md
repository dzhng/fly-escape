...
Object vocabulary: food fruit (odor plus edible landing surface), scent crumbs (odor without food), vinegar (repellent mapping), lamp/shade (measured visual cues), fan (wind), and threat cue. Levels introduce only objects whose graph-mediated effect passed the field/ablation probes. Fixed zappers are optional hazards, at most one per level, never at spawn or covering the only exit opening. Do not use hazard randomness to fake difficulty.
