...
## Timed campaign life policy — 2026-09-08

- **Keep finite energy as an explicit diagnostic policy — sound, medium confidence.** A campaign fly now starts with no modeled energy and can move until it escapes, meets a hazard or reaches the round timer. The lifecycle laboratory can still select a finite reserve to investigate feeding later. The user deferred that mechanic but did not specify whether to delete its useful diagnostics. One tagged life policy keeps the distinction explicit instead of giving campaign flies an arbitrarily huge reserve. Future energy experiments remain isolated from campaign rules.
- **Keep the recorded reserve field neutral in timed rounds — sound, medium confidence.** Timed flies record zero in the shared reserve slot; the campaign hides that value and the starvation counter. A laboratory recording still carries measured reserve. The alternative would require a second recording format or nullable field throughout replay. This choice preserves one recording format, but future consumers must use the authored life policy to interpret energy.
- **Test topology from the authored level modules — sound, high confidence.** Changing a doorway in the game now changes the map exercised by the topology test. The test reads the modules' JSON literals instead of maintaining a stale separate map fixture. The plan did not require a duplicate fixture; removing it avoids tests passing against a house players never enter. This assumes level content remains a JSON literal, which is already the authoring format.

## Source-oriented neural delivery — 2026-09-08

- **Reorient connections once when loading — sound, high confidence.** When a neuron stays silent, its outgoing connections now require no delivery work. The loader keeps one outgoing connection table instead of the incoming table used on disk. It visits firing source neurons in the same order that each receiving neuron previously used to sum its inputs, preserving floating-point results. The plan left internal performance choices open; this changes the runtime representation while preserving the download format and one shared graph. It adds temporary load-time indexing but no second permanent graph.
- **Read validated weights directly into their final positions — sound, high confidence.** During loading, weights go from the downloaded bytes into their outgoing slots. Keeping an intermediate full weight array raised allocator memory use, so the loader avoids that copy. This is a load-time implementation choice; validation still checks every weight, and the permanent array-size report remains accurate.
...
## Playback through rate fluctuations — 2026-09-08

- **Keep playing recorded frames when only the forecast worsens — sound, high confidence.** After a round has buffered enough to start, another application can briefly slow computation. Rechecking the entire predicted remaining shortfall can pause a replay that still has many seconds ready to show. The clock now keeps playing those frames; it pauses when the actual available recording falls below one wall second, then rebuilds the original conservative starting buffer. The user requested smooth buffered playback; the plan's earlier instruction to pause on a worse prediction was unnecessarily strict. A lasting slowdown can still require a visible pause, and every such pause remains counted. This changes interruption policy, not playback speed, recorded outcomes or the starting estimate.

## Second-house timed movement — 2026-09-08

- **Increase movement pace only in the larger house — sound, medium confidence.** At the original speeds, empty five-minute trials escaped no flies; doubling walking and flying speeds raised that to three or four. The second house now uses that tested pace, while the smaller first house retains its own setting. The user allowed faster movement when a map takes too long, but left the amount unspecified. Neural decisions, turn sensitivity and collision checks are unchanged, so the same motor output covers more ground. This improves traversal; it does not establish that the current placements or star thresholds are balanced. The change is a level-owned tuning value and can be revisited after fresh-seed comparison.

## Sensory-mechanics investigation — 2026-09-08

- **Isolate the decoder before combining candidate changes — sound, high confidence.** An odor experiment can change both the current sent into neurons and the way their activity turns a fly. Changing both together could improve escapes without showing which change helped. The next scratch comparison therefore keeps the existing sensory currents and changes only the olfactory steering readout; the bilateral-current candidate stays separate. The plan requires useful neural effects but leaves this experiment order open. This constrains experimental integration, not the final architecture: neither candidate becomes production behavior from a favorable two-seed score alone.

- **Keep the first firing-readout candidate unsmoothed — sound, medium confidence.** Counting firing neurons changes spontaneous turning as well as odor response. A smoother could reduce those small turns, but no actual play failure has been demonstrated. The candidate therefore retains a single fixed steering scale and immediate firing fraction while full-body and house checks run. The plan left decoding calibration open; this avoids introducing extra state or tuning controls before a real need exists. The user explicitly accepts unequal attraction and repulsion strengths.

## Replay disclosure and readable activity — 2026-09-08

- **Show recorded group activity on measured cell positions — sound, medium confidence.** The user requested a 3D brain and simultaneous traces. The existing recording contains group summaries, so the viewer colours measured members using those summaries and keeps other cells neutral. It omits missing anatomical positions rather than inventing them. This preserves truthful educational labels without expanding each replay into a full per-cell recording; individual cell activity remains outside this display's claims.
- **Award progress when the replay reaches its result — sound, high confidence.** Computation runs ahead to prevent stuttering. Saving stars at computation completion revealed future outcomes in the level menu. The player now earns the saved result when playback reaches its end, including seeking there, once per attempt. Rewinding keeps earned progress; cancelling before the reveal does not award an unseen result.

## Compact observation and departure presentation — 2026-09-08

- **Ghost the visible wall shell around a fly — provisional, medium confidence.** Physical wall boundaries are thinner than their decorative meshes, and readable flies are deliberately enlarged. Making the intersecting shell transparent reveals the fly without shifting its recorded body or changing collision behavior. Exterior grass can remain visible through it, reducing depth clarity. This uses the user's transparent-wall direction; it does not mean the fly is attached to a wall.
- **Use partial zoom compensation — sound, medium confidence.** The user requested gentle apparent growth instead of constant screen size. An exponent of 0.8 compensates most perspective shrinkage while preserving visible growth toward the camera; the prior world-size cap remains. The exact curve is a reversible presentation choice.
- **Render previews in one atlas — sound, high confidence.** All roster previews share one renderer and model instance, then copy individual tiles to small 2D canvases. The world's samples and camera orientation are their inputs, so a separate preview timer cannot drift from playback. This adds one WebGL context per active roster instead of one per fly.
- **Anchor the camera at the escape while the fly departs — sound, medium confidence.** A short outward climb and shrink makes escape visible without dragging the player away from the house. Eight simulated seconds bounds the visual lifetime and follows pause/seek. The clip cannot alter simulation results. A bounded presentation clock completes the remaining departure after the recording ends; visibility changes reset its timestamp so background time never advances it.

- **Force a multi-room route by closing one interior shortcut per house — sound, medium confidence.** Distant starts alone leave a route that can skip most of the house. The authored layouts now require three interior doorway crossings and retain a wrong-turn room. This is provisional level design pending actual playtests; it is not evidence that the object arrangements or star thresholds are balanced.

- **Keep exit assistance local and physical — sound, high confidence.** The exit pull uses the existing swept movement and physical crossing rules. It acts only within the authored radius, inside the exit room and with an unobstructed line to the opening. Both levels start with the same short range and strength, so reaching the goal room remains the player's task. The target sits just outside the opening to avoid a force that stops at the threshold. This is expressly requested assistance, not a claim that neurons guide the fly toward the exit.
- **Judge food as attraction plus retention — sound, medium confidence.** A food object may attract arrivals while also keeping them nearby. Escape count alone therefore cannot classify it as working or broken. Comparisons retain identical short-range exit assistance and omit fans, measuring approach, residence and departure as well as score. This treats the user's proposed strategy as a testable mechanic rather than assuming that adding more food must help.

- **Reject anatomically contradicted olfactory readouts at export — sound, high confidence.** When a hand-curated candidate is labeled as a descending neuron but the source calls it an internal brain cell, preparation now fails with its ID. The two known incorrect candidates are removed rather than replaced with cells chosen to produce better steering. The user required biological grounding but did not specify failure handling; failing before publication keeps mistakes visible instead of silently changing the modeled circuit. Validation includes listed candidates outside the reduced graph so an omission cannot hide a bad classification. Unequal left/right counts remain possible; equal counts alone would not establish biological symmetry.

- **Fit sensory input to published evoked-rate targets in an isolated timebase — provisional, medium confidence.** A measured half-second odor response provides an input target, but the game’s neural steps have no biological-second calibration. The scratch experiment assigns1ms per step locally and fits current using receptor firing only, then freezes it for new seeds. The source paper does not specify how to map into our simplified dynamics; this choice makes that assumption explicit rather than choosing current to improve escape. It supports an input approximation only, not production adoption, chemical concentration in air, or physiological timing. Exploratory adoption must disclose the abstract timing bridge and measured fitting error; a full physiological timing calibration is not required before playtesting.

- **Test geosmin-associated mold without promising avoidance — sound, medium confidence.** For the user's bounded repellent search, a moldy object stands for the microbial odor geosmin, whose sensory receptor circuit is identified in real fruit flies. The experiment stimulates that circuit using local modeled odor values, rather than reversing motor commands or labeling generic inhibition as a chemical response. The user left the candidate object open. This keeps a plausible avenue available for later exploratory play, but neither all mold nor this simplified input is a faithfully reproduced chemical mixture. The test is not a requirement to add a reliable repellent before finishing the game.


- **Allow a verified turn when translation is blocked — sound, high confidence.** The neural request already contains the turn; rejecting it solely because combined motion collides creates a physical fixed point unrelated to neural behavior. Hold the root for that substep, prove clearance throughout the turn, and preserve reachable terminal hazards. Ordinary contact can be deferred while turning. This conservative policy avoids adding steering forces or modifying neural outputs. The regression fails on the previous implementation; it does not establish that every prolonged object visit was a physics defect.

- **Retain explicitly exploratory vinegar wiring — sound, medium confidence.** The user permits existing neural pathways to remain available despite inconsistent responses and explicitly requested the name Vinegar. A later read-only audit suggested replacing its input with DA2 or removing the name; neither follows from that permission. DA2 evidence concerns geosmin, so applying it to vinegar would introduce another unsupported chemical claim. Preserve the current experimental binding and its disclosed limitations without promising repulsion. Chemical-specific validation remains incomplete; no new extraction or encoding pass is required by the completed optional search.

- **Give the first house ten simulated minutes — provisional, medium confidence.** The user expressly allows this timing lever. The exit-side placement rises from zero to four escapes on the matched browser/native seed between five and ten minutes. The completed browser replay preserves the one-minute Fast presentation with zero underruns. Startup is still too slow for release acceptance; this is a gameplay checkpoint, not accepted performance or final star balance. The second house retains its current timer pending its own evidence.

- **Use provisional first-level star thresholds of1,4,8 — rejected by user and reverted.** Recent complete attempts produce roughly2–7 escapes. Retaining8 for the second star and15 for the third would hide intermediate progress in those runs. One escape still unlocks the second level. Eight as a top-star target remains unvalidated against the desired player attainment rate; the choice improves current feedback without claiming a solved placement strategy or held-out balance.

- **Preserve the authored star thresholds — user-directed.** Both levels keep1,8,15. The brief first-level threshold adjustment was reverted on request; further mechanics and level work must preserve these thresholds.

- **Sound, medium confidence — preserve opaque back walls even when an enlarged fly overlaps their decorative shell.** Only foreground cutaways receive the near-fly fade. This restores the house silhouette; an overlapping fly may still be partly hidden against a back wall. Further presentation changes remain deferred while mechanics are calibrated.
- **Sound, medium confidence — isolate emission and sensory encoding before adopting stronger objects.** The sixteen-fly scratch matrix keeps exit suction and neural dynamics fixed, comparing one banana at normal/threefold emission under the current binary and one bounded concentration encoding. Two paired seeds are exploratory evidence, not a balance guarantee.

- **Sound, medium confidence — gentle exit-room air uses0.06 world units/second, combined with the stronger doorway pull by taking the larger speed.** This is authored physical assistance, not neural attraction. It acts only in the exit room with a clear line toward the opening and retains normal collision checks. Its balance remains provisional until actual-house playtests; objects must still bring flies into the final room.

- **Sound, medium confidence — retain normal emission consistently across objects while trying concentration-dependent sensory input in the browser.** Banana-only amplification isolated competition but is not a shipping exception. Global amplification did not improve placement advantage in the paired sample. The graded candidate is a modeled encoding, not measured receptor physiology; real browser control and balance still require verification.

- **Sound, medium confidence — accept graded odor at normal emission for the shipping candidate.** The bounded model retains side selection and neural ownership while giving concentration a tangible input effect. Its scale is a modeling choice, not receptor physiology. The current model has observable placement influence; extensive balance and startup optimization are explicitly deferred under the user's shipping decision.
