...
**Current pickup:** the user accepts food attraction plus distraction as a potential strategic mechanic and requests a direct simulation. Fan-free isolated-object runs show apple proximity/retention but no vinegar avoidance; [measurements](assets/evidence/32/odor-isolation/README.md). Keep identical short-range exit suction in all comparisons, restricted to the exit room; no placeable fans in these tests. The next experiment compares empty, one apple at the exit, and three food/leftover attractors along the route. Inputs/results: `/tmp/fly-food-strategy`; native PTYs60453/63004/14418 and browser PTYs19045/37075. Inspect live handles before continuing. Measure escape, proximity/grounded time and departure from food zones, not escape alone. No feeding/life-extension change is needed.

**Priority order:** finish the food strategy comparison and actual browser playtests; craft two playable levels and only then freeze setups/thresholds for tuning and holdout. The user-authorized exit suction is implemented and focused tests pass, pending final commit. It is physical assistance, not neural attraction. Vinegar's current weak pathway is a known limitation, not a solved repulsion mechanic; read-only analysis is in `/tmp/fly-vinegar-trace-output.txt`. Do not launch a neural redesign or fan-based workaround. Old long-run fan comparisons were stopped; their outputs cannot substitute for these tests. Holdout remains untouched.
