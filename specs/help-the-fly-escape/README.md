...
**Current pickup:** finish a bounded repellent-object experiment, then return to playable object placement in both houses. The user allows biologically motivated sensory wiring to remain exploratory even when responses are inconsistent or unexplained. Repulsion is desirable, not a release requirement. The current search is time-boxed to 30 minutes (2026-09-08 09:45–10:15 UTC), testing geosmin-associated mold with moving flies and matched no-stimulus controls. Existing DA2 neural observations are in [slice 33](slices/33-biological-sensory-grounding.md); they do not establish avoidance, but do not prohibit exploratory integration. Do not launch an open-ended circuit-completeness or physiological-calibration program.
