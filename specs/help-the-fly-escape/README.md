...
**Current pickup:** act on the [hands-on browser playtest](assets/evidence/30/hands-on-playtest/README.md) before more model tuning. Three current-build rounds completed: first house12escapes/2stars with apple, then7/1 with fan; second house1/1 with apple/banana/vinegar. Retry, two replay speeds and card selection work. Camera browser checks now pass model picking. The early stars/unlock reveal is fixed: progress is awarded only when replay reaches the end, with a red-to-green browser regression (`tests/browser/result-reveal.mjs`). Keep visual work limited to functional playability.
