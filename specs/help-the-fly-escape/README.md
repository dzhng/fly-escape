...
**Current pickup:** finish the sixteen-fly clustered-versus-spread placement comparison, then choose the simplest supported sensory/object policy for browser playtesting. The [scent-strength matrix](assets/evidence/33/scent-strength/README.md) is complete: normal→threefold banana emission improved escapes in both paired seeds under both encodings. Binary totals were5 empty /3 normal /9 stronger; concentration totals3/6/11, each out of32. The candidate changes both attractive and repellent input and lowers finite-concentration drive, so cross-encoding outcomes are not a pure sensitivity comparison. Production encoding/emission remain unchanged. Claude's private run `/tmp/fly-scent-strength` may still be preparing its report; simulations are complete. Root verified raw complete results and source hashes rather than trusting partial aggregate summaries.

The cluster comparison uses `/tmp/fly-maze-duration/target/release/examples/cluster_probe` with16 actual flies, the original binary adapter and doorway-only pull. Inputs/output are in `/tmp/fly-cluster-comparison`. Seed0 cluster and spread jobs are running; verify current process liveness before waiting/restarting, collect results, then run seed1. Both configurations use five normal bananas and differ only in positions. Do not mix these diagnostic runs with the integrated room-pull build. Preserve star thresholds1/8/15 and the existing timer.
