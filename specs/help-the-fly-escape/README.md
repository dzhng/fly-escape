...
**Current pickup:** user clarified that biological fidelity precedes game design. Vinegar is not a generic fruit-fly repeller: low-concentration apple cider vinegar attracts real Drosophila; concentration and state matter. The current Vinegar → RepellentOdor → inhibitory LH input binding is an assumption, not a validated chemical receptor model. Inhibitory signaling is not synonymous with behavioral avoidance. Audit the sensory binding against primary evidence before further object tuning; see [biological grounding](assets/evidence/32/food-strategy/biology.md). The food-strategy runs establish behavior of the current model, not biological validation. A ten-minute route-food/empty diagnostic is already running (native PTY48174, `/tmp/fly-food-strategy/route-food-10min-results.json`); read-only content consultation has completed in `/tmp/fly-food-content-consult.txt`. The biological audit completed in `/tmp/fly-sensory-biology-audit.txt`; root annotation joins are banked with the biological grounding report. Two right olfactory readout members are annotated intrinsic brain cells, and the existing negative DM1 substitution must not be repeated. Check the native handle rather than restarting it; neither authorizes model tuning to force a desired outcome.
