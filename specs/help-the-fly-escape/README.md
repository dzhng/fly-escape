...
Continue [20 — furnishing and food contact](slices/20-furnished-contact.md) with the measured rotating face-to-edge transition. Same-triangle vertex handoffs have conditional whole-interval certificates; the next seam requires edge contact. Resolve that bounded seam, then exercise actual requests through the existing body and recording owners in an isolated diagnostic. Expand from reproduced failures rather than building unrestricted feature/event machinery first. [Current path evidence](assets/evidence/20/support-path-feasibility/README.md) owns the numerical qualifications and next proof.
