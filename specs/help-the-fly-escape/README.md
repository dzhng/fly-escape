...
Continue [20 — furnishing and food contact](slices/20-furnished-contact.md), starting with a continuous supported path that can turn, cross surface features and leave an apple without penetration or zero-time cycling. The endpoint-projection movement prototype is **not accepted**. It is isolated in `/tmp/fly-supported-motion` on `codex/supported-motion`, with uncommitted body/surface changes and an apple-edge regression. Root remains on the verified recording seam. Use the [path feasibility evidence](assets/evidence/20/support-path-feasibility/README.md): analytic local face walks and the edge handoff now work, but global competing contacts and general translating rotation remain unaccepted. The [maintenance review](assets/evidence/20/support-path-feasibility/maintenance-review.md) identifies the next pass: compare smaller outward-conservative native hulls before expanding the exact event solver. Preserve containment, visible contact, world scale and numerical tolerance; the current prepared hull is an oracle, not a fixed requirement. Smaller substeps alone still do not prove a safe path.
