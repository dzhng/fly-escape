...
**Live comparison checkpoint:** post-contact sparse-food run is active in native `duration_probe` (session93417; output `/tmp/fly-food-post-contact-A.json`, log `/tmp/fly-food-post-contact-A.log`). It checkpoints each arm and compares no added objects with a banana near the final-room entrance for three seeds over6,000ticks. First complete pair is5 versus2 escapes; do not generalize until the remaining pairs finish. Revalidate the existing process before any restart. Scratch source `/tmp/fly-maze-duration` includes the production contact fix; production graph and fan-free configuration are unchanged. This run is balance evidence, not a new neural calibration.
