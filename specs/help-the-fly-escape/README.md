...
Continue [20 — furnishing and food contact](slices/20-furnished-contact.md). Numerical body trajectories, lifecycle transitions and [shared packed replay](assets/evidence/20/numerical-replay/README.md) are integrated and pass their focused checks. The next blocker is reliable, affordable movement on food under actual neural and fan requests: measure trajectory error, all-neighbor contact, forward progress and twenty-body WASM cost before accepting furnished food. Formal interval certificates are rejected as the default movement method; retain their research as evidence.

The isolated contact worktree `/tmp/fly-numerical-contact` has prepared straight-path coalescing and is resolving strong-wind query cost. Integrate reviewed changes without replacing the shared trajectory/archive owner. Curved-food visual acceptance, full-horizon archive consumption and production cost remain open. Small height error alone cannot establish support across a gap.

Next finish20, then household shapes/materials/light/readability21–24, the five-room first level15, the larger second level16 and release17. Prepared household assets pass [browser replacement and inspection](assets/evidence/21/browser-shapes/README.md); furnished-game placement remains open. Grass and campaign UI are prepared in isolated worktrees; integrate useful work instead of rebuilding it. Do not recalibrate campaigns until contact and room geometry settle.
