...
**Current pickup:** integrate the bounded graded-odor implementation running in `/tmp/fly-graded-odor` (Claude transcript `/tmp/fly-graded-odor-claude.txt`; verify liveness before waiting). Its only allowed production owner is `crates/sim/src/sensory.rs`, including focused tests; normal emissions and binary Vision are preserved. Root browser baseline with one banana and current room pull is running via `/tmp/fly-graded-baseline-banana.mjs`, output `/tmp/fly-graded-baseline-banana`; the matching empty baseline is banked under final-room-pull evidence. Review the candidate diff, build WASM/web, then repeat both actual browser setups with graded odor input before acceptance. Keep the change reversible and explain that sensory encoding is modeled; do not add an object-specific banana boost or adopt global3× as an assumed improvement. The candidate scales the detected winning side by c/(c+0.55), retaining detector, signs, dynamics and readout exclusion. Reconcile tests around dose response, side identity, bounded input and motor exclusion rather than preserving binary amplitudes. Run full relevant checks and independent review before committing. Preserve star thresholds1/8/15 and timers. Actual campaign balance and release performance remain open.

**Latest evidence:** [banana-only emission](assets/evidence/33/scent-strength/README.md) improved escapes in both paired seeds under both encodings, but was an isolation experiment rather than an intended asymmetric shipping rule. [Global3× emission](assets/evidence/33/global-scent/README.md) is complete: binary totals5 empty /3 banana at both scales; concentration totals3/6 at normal emission and8/7 at global3×. This does not support global amplification as better placement control. [Clustered versus spread bananas](assets/evidence/33/clustered-bananas/README.md) is complete:6 versus9 escapes, so this cluster did not outperform spreading. All these diagnostic runs use16 flies, old doorway-only suction and two seeds; they are exploratory evidence, not accepted game balance. The concentration candidate lowers effective attractive drive as well as adding dose dependence. This particular house has no active repellent emitter, although the code supports inhibitory odor encoding. All experiment processes are terminal; do not restart them. Scratch candidate `/tmp/fly-scent-strength` is available for reference; remove scratch switches/wrappers when implementing one production owner.
