...
**Current pickup:** the user rejected the old same-room escape arrangement as maze gameplay. Both levels now start far from the exit and require three interior doorway crossings through four rooms, with dead ends retained. Actual browser placement/release attempts on the new first-house route escaped 1/20 and 0/20 in five minutes; these are playtests, not balance acceptance. The valid same-seed/browser-placement duration comparison escaped 0/20 at five minutes and 3/20 at ten minutes; initial bodies and placements match exactly. Ten minutes buffered for100.45seconds, so longer duration alone is not accepted balance or performance. [Reports and limits](assets/evidence/32/maze-duration/README.md). The continuous native diagnostic is live in `/tmp/fly-maze-duration` (Claude PTY46795), outputs `/tmp/fly-maze-duration-results/`: matched empty/placed arms at5,10,20,30,60minutes. Its scratch-only body duration cap is authorized; chained/restarted attempts are explicitly rejected because they reset state. The old empty-control process was stopped and its old-layout results must not calibrate these routes. Holdout remains untouched.

**Priority order:** integrate local exit suction (Claude PTY45593, `/tmp/fly-exit-suction`), verify wall occlusion and normal physical crossing, then compare it disabled/enabled on the actual maze. Continue placement/release/observe/retry playtests before freezing setups and thresholds. Only then run matched tuning/holdout and sustained release checks. Keep neural dynamics, normal contact and recorded replay; the user-authorized local exit pull is physical assistance, not neural attraction. Feeding and art polish remain deferred.
...
- [Current maze duration](assets/evidence/32/maze-duration/README.md): browser five/ten-minute pair is complete; continuous native controls are still running. First-seed native ten-minute checkpoints show6empty versus3placed escapes. Do not label the current arrangement a successful guide.
