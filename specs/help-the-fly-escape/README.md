...
Active implementation: Claude in `/tmp/fly-timed-rounds` owns explicit timed body policy, campaign durations and generated types; Claude in `/tmp/fly-two-speed-replay` owns the two-mode clock/UI and related tests. Root integrates and verifies, updates science cards to omit inactive reserve, then runs short timed-round placement pilots before tuning/reference/holdout validation. Do not run the prepared old finite-energy warmup confirmation: its regime is superseded. Production initialization remains cold; the new much-longer timed regime needs its own evidence.
