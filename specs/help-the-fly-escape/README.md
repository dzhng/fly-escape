...
**Current pickup:** freeze reference/poor arrangements for the integrated firing readout and perform campaign calibration. [Message-based Worker yielding](assets/evidence/17/task-yield/README.md) reduces matched production startup from37.7s to18.4s with identical recording checksums and zero underruns; cancellation/hidden transport checks pass. This closes that measured overhead, not sustained release acceptance. Actual integrated browser play is complete; do not repeat old model-response spikes or resume visual polishing.
