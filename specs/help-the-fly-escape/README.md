...
Continue [20 — furnishing and food contact](slices/20-furnished-contact.md) with rotating contact-chart interval certificates, measuring certified coverage and explicit unresolved cases before expanding event machinery. The [fixed-orientation prefix component](assets/evidence/20/support-path-feasibility/fixed-prefix-component.md) is integrated and verified under the existing query owner. It consumes the prepared original-plane boundary, preserves the supplied start and stops at gaps, discontinuities or neighbors. Its logical work/item budgets and native measurements are component evidence, not a browser performance guarantee. The shape remains a candidate, not an adopted body. Continue [rotating/global path work](assets/evidence/20/support-path-feasibility/README.md), preserving native scale, visible contact and numerical tolerance; small geometric uncertainty alone cannot prove continued support across a gap.

The endpoint-projection movement prototype is **not accepted**. It remains isolated in `/tmp/fly-supported-motion` on `codex/supported-motion`, with uncommitted body/surface changes and an apple-edge regression. A supported path must turn, cross features and leave an apple without penetration or zero-time cycling; smaller substeps alone do not prove this. Root retains the verified recording seam.

Verified foundations are the normalized query kernel, support-root sampler, shared authored food geometry, native floor height and [integrated support/orientation records](assets/evidence/20/support-record/README.md). Root currently writes upright orientation and null food support. The native animation hull is a prepared candidate, not an accepted moving body. [Native furniture identity](assets/evidence/20/furniture-identity/README.md) is integrated on existing solids, with shared catalogue dimensions and quarter turns; final furnished appearance remains open. Curved acquisition, walking, feeding, departure/revisit, replay/ring conformance and twenty-brain integration remain open. Keep one body/query owner; do not restore renderer height workarounds, substitute the test ball, retune neural gains or rebuild per-fly triangle scenes.
...
Physical adoption25 is accepted after continuous sampling26 and sensitivity27; wide zoom readability28 is accepted with native close pixels unchanged. Bare-room campaign evidence does not validate the new physical/furnished levels: do not restart campaign batches until contact and room geometry settle. The isolated campaign progression implementation awaits accepted content; integrate it instead of rebuilding it. Renderer retention is patched; final release performance/platform measurements remain open. Component evidence stays in each owning slice. Cabinet/sofa native loading is integrated; the [refined plant](assets/evidence/21/plant-refined/README.md) and [banana](assets/evidence/21/banana-prepared/README.md) remain prepared shape studies pending refinement and contact/room integration. They reuse the shared authoring stage.
