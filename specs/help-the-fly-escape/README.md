...
Continue [20 — furnishing and food contact](slices/20-furnished-contact.md) with a bounded fixed-orientation support-path component under the existing query owner. The [prepared original-plane boundary](assets/evidence/20/support-path-feasibility/native-envelope/preparation.md) is committed; an independent exact-integer audit confirms the saved metre planes' vertex/edge connections. Reconstructed QuickHull feature topology fails, so consume the saved inventory. Return only a continuously supported prefix, with explicit gap/discontinuity/neighbor stops and bounded work; preserve the supplied start instead of lifting onto a new branch. The shape remains a candidate, not an adopted body. Then continue [rotating/global path work](assets/evidence/20/support-path-feasibility/README.md), preserving native scale, visible contact and numerical tolerance.
