...
**Latest user-requested presentation:** per-fly zoom sizing and right-drag/reset camera, 5× wing clip, skull/escape icons, exit glow, measured 3D soma view and all-group traces are implemented. The brain now uses the panel’s light background, with [production captures and WebKit resize verification](assets/evidence/30/brain-theme/README.md). Camera and science browser suites pass; [evidence and limits](assets/evidence/30/reading-flies/README.md). The optional [geometry-input spike](assets/evidence/30/geometry-vision/README.md) showed no clear benefit and remains out of production. The subsequent firing-readout integration and normal-UI checks are recorded below.

**Current pickup:** first-house30-seed tuning is running in PTY session27860, using `/tmp/fly-house-two-target/release/examples/campaign_probe`; output `/tmp/fly-first-tuning30-verified.json`, log of the same stem. Verify the handle before continuing. The [frozen placements and preliminary evidence](assets/evidence/15/integrated-placement-pilot/README.md) distinguish an interrupted binary-provenance diagnostic from this clean-build run. Do not restart the terminal earlier session66639 or any Claude session. The [second-house route pilot](assets/evidence/30/route-placement-pilot/README.md) completed all four trials:5/3escapes versus2/0, median advantage3; no campaign acceptance. Next inspect the completed first-house distribution and decide the next second-house calibration from these results. Holdout remains untouched. Keep buffer safety, neural rules and accepted visuals stable.

**Integration shipped:** `3f3af05` integrates the supported firing readout and source binding with reference parity and browser verification. Claude session66493 is terminal. [Integrated playtest evidence](assets/evidence/30/integrated-playtest/README.md) records both completed house rounds; it does not establish improved campaign balance. The [moving chamber](assets/evidence/30/spike-steering/README.md), [full-body responses](assets/evidence/30/spike-body/README.md) and [eight timed house runs](assets/evidence/30/spike-campaign/README.md) remain the mechanical evidence. Weak/asymmetric inhibition is acceptable; no smoothing, strength matching or new neural sweeps. [Startup delivery evidence](assets/evidence/17/startup-delivery/README.md) preserves a failed frame-time gate and shows why buffer padding alone cannot resolve production cost. Startup performance and campaign calibration remain open.
...
**Performance:** [sparse delivery](assets/evidence/17/sparse-delivery/README.md), [shared trigonometry](assets/evidence/17/shared-trig/README.md) and [buffer handling](assets/evidence/17/buffer-fluctuations/README.md) are integrated. The newer [task-yield comparison](assets/evidence/17/task-yield/README.md) cuts initial Fast wait below target on the measured run without changing buffer policy; Firefox smoke passes as well. Final sustained release and platform gates remain open.

**Priority order:** freeze reference/poor setups and star thresholds; paired30-seed tuning and disjoint30-seed validation; sustained production memory, cancellation, input, hidden-tab and platform checks. Spatial response is supported by the completed experiments above. Empty/fan-only setups are diagnostic controls, not replacements for the required deliberately poor legal setup. Preserve native contact and recorded replay; no invisible food enlargement or target steering. Actual Safari and the older microscopic contact-interpolation limitation remain open. Do not resume feeding or cosmetic refinements.
