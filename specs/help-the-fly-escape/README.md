...
Parallel bounded work: The [timed placement pilot](assets/evidence/30/timed-placement-pilot/README.md) is complete (session32683 terminal):15attempts, best candidate mean10escapes versus8empty, with all benefit confined to one of three seeds. Next isolate exit-adjacent apple and relocated vinegar on fresh tuning seeds; this is not final balance. The named-WASM diagnosis is complete and banked in [neural-cost evidence](assets/evidence/17/neural-cost/README.md); session45467 is terminal. Its noise-stream-change recommendation was not adopted. A source-oriented neural-delivery implementation runs in `/tmp/fly-sparse-neural-delivery` from8ae56ea (session29886; artifacts `/tmp/sparse-neural-delivery/`). It must preserve floating-point accumulation order, RNG and all neural values, replace rather than duplicate runtime matrix storage, and demonstrate a measured speed improvement before integration. Root's native sample `/tmp/timed-native-sample.txt` identified Brain::advance as the dominant self-time. Verify each process/report before adopting conclusions.
