...
Parallel bounded work: Claude's timed placement pilot runs in `/tmp/fly-timed-placement-pilot` (session32683; artifacts `/tmp/timed-placement-pilot/`), comparing legal empty/apple/vinegar/combined plans on three fixed seeds. Named-WASM performance diagnosis runs in `/tmp/fly-timed-rounds` (session45467; artifacts `/tmp/timed-performance/`). A source-oriented neural-delivery implementation runs in `/tmp/fly-sparse-neural-delivery` from8ae56ea (session29886; artifacts `/tmp/sparse-neural-delivery/`). It must preserve floating-point accumulation order, RNG and all neural values, replace rather than duplicate runtime matrix storage, and demonstrate a measured speed improvement before integration. Root's native sample `/tmp/timed-native-sample.txt` identified Brain::advance as the dominant self-time. Verify each process/report before adopting conclusions.
