...
Next resolve actual native-hull acquisition, walking, feeding, loss, departure and revisit, then curved replay and selection-ring conformance. Food proximity still uses the provisional body radius; it is not apple landing/walking acceptance. Floor meal and neural-ablation checks pass, but curved food movement and20-brain integration remain open. Do not rebuild the query owner, restore renderer-only height or use the test ball as the fly body.
