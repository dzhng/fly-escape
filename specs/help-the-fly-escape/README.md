...
**Current pickup:** establish useful attraction before further placement sweeps. The [odor readout probe](assets/evidence/30/odor-readouts/README.md) rules out a walking/flying polarity reversal and finds opposing voltage/firing changes. A bounded scratch firing-based steering comparison is running in `/tmp/fly-sparse-neural-delivery`, Claude session30730, output `/tmp/spike-readout-output.txt`, artifacts `/tmp/spike-readout/`. Verify liveness and inspect raw movement results before adopting anything. Production decoding remains unchanged.
