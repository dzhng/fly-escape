...
The [second-house shoe-rim failure](assets/evidence/30/refinement-contact/README.md) is resolved in native and rebuilt browser runs of the captured seed; unresolvable supported motion retains a verified safe prefix without changing precision or work limits. Continue sustained random-attempt checks, then finish the two-level game through [30 — Household objects and hazards](slices/30-household-objects.md), then campaign calibration and final release gates. Exactly two crafted levels, twenty flies, one fan maximum, object discovery and meaningful eating remain the target. Do not return to chained-fan puzzles or the rejected microscopic contact-certificate program.
