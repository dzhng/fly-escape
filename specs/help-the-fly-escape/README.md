...
**Current pickup:** validate the [firing-based steering candidate](assets/evidence/30/spike-steering/README.md), which produces clear mirrored attraction and weaker repulsion in the moving chamber. Production decoding is unchanged. Two bounded scratch tasks are running: full Attempt/Body mirrored odor responses in `/tmp/fly-sparse-neural-delivery` (Claude session22007, output `/tmp/spike-body-output.txt`, artifacts `/tmp/spike-body/`); eight second-house attempts in `/tmp/fly-spike-campaign` (session24071, output `/tmp/spike-campaign-output.txt`, artifacts `/tmp/spike-campaign/`). Verify liveness and raw results before integration. Both use firing scale four and reverse source-to-pathway bindings to match measured polarity; neither includes bilateral odor presence or smoothing. Integration and focused tests run separately in `/tmp/fly-spike-integration` (Claude session65458, output `/tmp/spike-integration-output.txt`). Root owns the final diff review and production-browser check.
