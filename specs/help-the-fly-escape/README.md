...
Next priorities: finish the sustained browser gates (05); camera/selection (06) passes functional tests, while dense-fixture marker clarity remains open. Real-model workbench/integration (07) is under visual review; dependent acceptance remains gated. The Rust packed format, WASM session and client archive/clock have focused tests; real-browser transport checks pass hidden/cancel/restart/ownership/determinism. [UI integration](assets/evidence/05/ui-review.md) is verified; sustained browser performance remains open. Blender fly source/GLB and four motion clips are prepared and checked in Blender and Three.js; [Motion sampling preparation](assets/evidence/08/motion-review.md) now uses the same cursor in playback and the workbench; acceptance remains gated by dependencies and final visual review. Core mixed-cue preparation is committed as 98c9137 in its isolated worktree and awaits integration after the running baseline benchmark; final acceptance keeps its original dependencies. Nothing beyond 04 is a completed slice. Keep one owner per concept and preserve the complete active-work benchmark; do not count terminal no-ops as production throughput.
