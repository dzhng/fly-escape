...
Next replace planar edible circles with one authored mesh/contact authority and a prepared scene shared by the attempt's flies. Then resolve actual supported movement/identity and curved interpolation, keeping the measured native hull provisional until moving contact proves it. Follow the remaining passes in20; do not rebuild the query layer, restore display-only height, or adopt the test ball as a fly body.
