...
**Current pickup:** isolate scent emission from neural encoding using the user-requested sixteen-fly diagnostic runs. The present adapter turns detectable left/right contrast into fixed current, discarding concentration. The user authorized testing normal versus threefold object emission and concentration-dependent input. Keep the existing exit suction identical, use no fans, preserve neural dynamics and star thresholds. A bounded private comparison is running in `/tmp/fly-scent-strength`, with output under `/tmp/fly-scent-strength-results` and Claude transcript `/tmp/fly-scent-strength-claude.txt`; verify process liveness before waiting or restarting. Production sensory behavior is unchanged pending evidence.
...
**Independent pending work:** user-authorized gentle final-room pull is implemented and independently reviewed in `/tmp/fly-final-room-pull` (review `/tmp/fly-final-room-pull-review.txt`, patch `/tmp/fly-final-room-pull-reviewed.patch`), but is not integrated. Apply the diff rather than copying the older level files; preserve current thresholds and timer. Keep this pull out of the scent comparison. [Foreground-only wall fading](assets/evidence/31/foreground-walls/README.md) is verified; matched browser frames show opaque back walls, with an unprimed critique noting remaining translucent bands, small selected flies and conspicuous terminal trails. Those residual presentation issues are deferred under the user's mechanics-first priority; do not claim a fully unobstructed cutaway.

**Priority order:** plausible sensory wiring and meaningful aggregate placement control, then two playable levels and star thresholds, then final performance/platform gates. Preserve variable responses and honest model limitations; do not tune neural signs to manufacture predetermined behavior. Short-range exit suction stays identical across comparisons and restricted to the exit room. Test without fans first; the single fan cannot substitute for useful object-placement mechanics. Holdout remains untouched. Feeding and visual polish remain deferred.
