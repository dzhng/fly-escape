...
The timed-round candidate is integrated in the working tree: an explicit body life policy disables campaign energy spending, feeding and starvation while preserving physical landing/walking and hazards. Both houses use five-minute horizons. Focused tests, the full native suite, generated types, TypeScript and WASM/application builds pass. Both houses completed full Fast attempts on the static production build with no starvation or underruns; see [timed integration evidence](assets/evidence/17/timed-rounds/README.md). Startup took84–98seconds and fails the performance target. Independent review, root cleanup and final rebuild are complete. The timed policy is ready; startup performance and balance remain open. Claude's timed implementation process is complete; no old session should be resumed. Production initialization remains cold; do not run the obsolete finite-energy warmup confirmation.
...
Parallel bounded work: The [timed placement pilot](assets/evidence/30/timed-placement-pilot/README.md) is complete (session32683 terminal):15attempts, best candidate mean10escapes versus8empty, with all benefit confined to one of three seeds. Next isolate exit-adjacent apple and relocated vinegar on fresh tuning seeds; this is not final balance. The named-WASM diagnosis is complete and banked in [neural-cost evidence](assets/evidence/17/neural-cost/README.md); session45467 is terminal. Its noise-stream-change recommendation was not adopted. The [source-oriented neural delivery](assets/evidence/17/sparse-delivery/README.md) is integrated in the root working tree from Claude commitbca3e40; session29886 is terminal. Native reference/equivalence checks and actual browser playback pass. Startup on the captured second-house seed improved from98seconds to48seconds, with unchanged final screenshot and zero underruns. It preserves RNG and all neural arithmetic, replaces rather than duplicates runtime matrix storage, and still needs further startup improvement. No Claude processes remain active.

Remaining order: profile slow production and establish tangible object-placement effects and tune both puzzles within ten minutes; run fresh-seed reference/poor-placement gates; finish static-release performance, memory, cancellation/input/hidden-tab and platform checks. Physical contact/replay remain required; feeding benefit/revisit/energy ablation and cosmetic refinements are deferred. Prior component measurements and browser smoke are not final release acceptance. Actual Safari remains untested. The older microscopic interpolation issue remains explicit in contact evidence.
