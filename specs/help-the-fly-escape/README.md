...
**Latest user-requested presentation:** per-fly zoom sizing and right-drag/reset camera, 5× wing clip, skull/escape icons, exit glow, measured 3D soma view and all-group traces are implemented. Camera and science browser suites pass; [evidence and limits](assets/evidence/30/reading-flies/README.md). The optional [geometry-input spike](assets/evidence/30/geometry-vision/README.md) showed no clear benefit and remains out of production. Firing-readout integration is isolated in the worktree below; no simulation decoder change shipped with this presentation pass.
...
**Integration resumed:** `/tmp/fly-spike-integration` contains the firing-decoder/source-binding candidate. After the required three browser rounds, Claude session66493 resumed the bounded implementation to finish reference parity, browser consumers and contracts. Verify that session/worktree state before restarting anything. No production integration or new balance claim has occurred. The [moving chamber](assets/evidence/30/spike-steering/README.md), [full-body responses](assets/evidence/30/spike-body/README.md) and [eight timed house runs](assets/evidence/30/spike-campaign/README.md) are complete. All prior experiment agents are terminal; the integration handle above is separate. Weak/asymmetric inhibition is acceptable; no smoothing, strength matching or new neural sweeps. After integrating the supported mechanic with its contracts/tests, play it in the browser again before further balance changes.
