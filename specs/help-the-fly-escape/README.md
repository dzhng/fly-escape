...
**Current pickup:** [biological sensory grounding](slices/33-biological-sensory-grounding.md) precedes object tuning. The [fixed-input laterality check](assets/evidence/33/da2-laterality/README.md) is complete: receptor stimulation is lateralized, but the current steering readout and separately observed DNa01/DNa02 do not establish mirrored direction. No experiment is live. The DA2 input/graph remains scratch; the production model still bypasses odor receptors. Stop gain/decoder variants on this branch. Next inspect the specific structural limitation of seed-touching extraction (measured connections between selected neighboring neurons are omitted) before deciding whether a bounded circuit-completeness comparison is warranted. Keep negative results and require an independent modeling reason for any experiment.
