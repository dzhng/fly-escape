...
Continue [20 — furnishing and food contact](slices/20-furnished-contact.md), starting with its food-geometry/identity pass. The core query kernel and fixed support-orientation binding are verified. [Physical floor height and replay](assets/evidence/20/vertical-motion/README.md) are now verified too: Landing remains airborne until touchdown, schema2 records native height, and camera-scaled trails include vertical motion. The controlled floor-meal gate passes after adding descent runway to its food patch; this is not curved-food or campaign acceptance.

The [neutral authored apple](assets/evidence/20/authored-apple/README.md) now provides identical Blender and core triangles; fixed-pose inspection exposes planar-ring clipping in its indentation, which remains20 work. Next replace planar edible circles with one authored mesh/contact authority and a prepared scene shared by the attempt's flies. Then resolve actual supported movement/identity and curved interpolation, keeping the measured native hull provisional until moving contact proves it. Follow the remaining passes in20; do not rebuild the query layer, restore display-only height, or adopt the test ball as a fly body.
