...
**Current pickup:** repair the [contact turn fixed point](assets/evidence/20/contact-turn/README.md), then rerun sparse food placements before balance changes. A blocked translating sweep currently discards an independently clear in-place turn, freezing a fly beside an object and inflating dwell measurements. The fix must record only the brain-requested, swept-clear rotation with translation held; no steering or neural retuning. The current [browser sparse-food attempt](assets/evidence/15/sparse-food-playtest/README.md) completes, but placement advantage is not established. The optional [repellent search](assets/evidence/33/repellent-object/README.md) is complete and must not be reopened as a release blocker.
