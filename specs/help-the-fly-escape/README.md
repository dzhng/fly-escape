...
Priority one is playable mechanics: meaningful average escape improvements from objects, useful eating, then two well-designed levels. The user accepts the current visuals for MVP; no further cosmetic work is required. Claude handles bounded experiments and implementation in isolated worktrees; root reviews and verifies.

The [contact-acceptance repair and complete pilot](assets/evidence/30/contact-acceptance/README.md) are the current pickup. All48 first-house pilot cells complete after the repair;77 focused native tests and four captured production-browser attempts pass. Warm60 apple-plus-vinegar averages4 escapes versus1.33 empty; cold averages2.33 versus0.5. This is a six-seed pilot, not level acceptance. Run the same eight conditions on fresh seeds200–229, keeping content/placements unchanged and preserving exact sources. No warmup or simplified food controller has shipped. Do not choose gains or placements using the confirmation outcomes and call them held out.

Food is still rarely encountered: only one fly-life fed in each24-run initialization arm. Physical replenishment is verified on [native banana](assets/evidence/20/native-banana-meal/README.md), and the new warm pilot includes repeated meals extending a fly to the attempt horizon. That does not establish reliable food-seeking. The [feeding audit](assets/evidence/30/feeding-path/README.md) identifies a missing historical approaching-food input, but the existing loom group is LC4 and cannot be presumed suitable for landing. Preserve sensory-only injection and motor readout exclusion; no invisible food enlargement or target steering. Earlier [warmup/odor evidence](assets/evidence/30/warmup-response/README.md) remains scoped to its tested model and conditions.

The two furnished houses, random mixed starts, one fixed-heading fan maximum, object images, discovery-focused UI, neuron explanations, RTS camera/follow and recorded replay are integrated. Native [zapper](assets/evidence/30/native-zapper/report.md) and [web](assets/evidence/30/native-web/README.md) hazards are integrated. Current second-house captured runs finish but all flies starve; reliability is not puzzle solvability. A separate content review's nominal6m travel limit ignored neural thrust and wind, so do not change layout or reserve on that assertion. Validate candidate placements through the actual placement owner before simulation.

Remaining order: confirm object effects; resolve useful food encounters/revisit; calibrate and validate both puzzles, including food-energy ablation on the larger map; finish static-release performance, resource/cancellation/input/hidden-tab and platform gates. Prior [production measurements](assets/evidence/17/domestic-production/README.md) and [Firefox/WebKit smoke](assets/evidence/30/browser-engines/README.md) are component evidence; actual Safari remains untested. The pre-existing microscopic interpolation issue is still explicit in the contact evidence. A green committed pass is a checkpoint; continue through every open item.
