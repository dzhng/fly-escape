...
**Current pickup:** food placement is the core strategy: attraction can help progress, while lingering at competing destinations costs time. The completed [fan-free food comparison](assets/evidence/32/food-strategy/README.md) supports placement sensitivity but not final balance. Adding vinegar beside the fixed off-route banana did not improve escape counts or consistently reduce Study residence. All native and browser jobs for this comparison are complete; no live handles remain. Household props count as attractors only when backed by simulation inputs, not merely visible decoration.

**Priority order:** craft two playable levels around measured food effects, testing clock/layout changes within the ten-minute playable limit, then freeze setups and thresholds for tuning and holdout. Short-range exit suction is shipped at5b3de25 and stays identical across comparisons, restricted to the exit room. A single fan may be tested as a counter to distractions, but must not replace fan-free controls. Vinegar remains an unproven counter; do not launch a neural redesign or represent it as solved. Holdout remains untouched. No feeding/life-extension change is needed.
