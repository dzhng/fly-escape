...
**Current pickup:** return to fan-free object-placement playtesting and finish two playable levels. The [bounded repellent search](assets/evidence/33/repellent-object/README.md) is complete: geosmin-associated mold changes trajectories but has not demonstrated consistent avoidance in moving chambers or the authored house. No experiment is live. The DA2 graph/wiring remains scratch, with its assumptions and reusable evidence preserved; the browser still uses its existing sensory bindings. The user permits exploratory circuits with inconsistent or unexplained responses, and reliable repulsion is optional. Do not resume the repellent search, a circuit-completeness investigation or exhaustive physiological calibration. Use the available attraction/distraction effects to test useful placement strategies, with the same local exit suction and no fan masking the comparison.
