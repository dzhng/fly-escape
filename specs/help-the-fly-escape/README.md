...
**Current work:** the second house now uses the verified faster movement pace ([production evidence](assets/evidence/16/timed-pacing/README.md)). Its doorway fan plus rear vinegars averaged4.75escapes across four [fresh tuning seeds](assets/evidence/30/fresh-doorway-seeds/README.md), versus3.25empty and3.5fan-only. First-house trials remain in [object comparison](assets/evidence/30/timed-fan-comparison/README.md). These are candidates, not final balance.

Current bounded work: `/tmp/fly-second-house-timed-pilot`, session53749, artifacts `/tmp/second-house-gain/`, tests existing odor cue gains3 instead of2 (six trials; no production change). Root food-only ablation runs from `/tmp/second-house-food-only/` on110/111, sessions11015/86415, to separate the corridor fan from the study-food setup. Verify handles before treating them as live. Completed [turning-response trials](assets/evidence/30/turning-response/README.md) gave combined9/1 versus current4/6; turnGain remains8. The [study-food pilot](assets/evidence/30/study-food-pilot/README.md) scored2/5 and is not accepted as reliably poor. No confirmation processes remain paused.

Simulation and playback improvements are integrated: [shared WASM trigonometry](assets/evidence/17/shared-trig/README.md) preserves exact recordings; [buffer-fluctuation handling](assets/evidence/17/buffer-fluctuations/README.md) avoids prediction-only interruptions while retaining genuine-depletion protection. Full Chrome runs pass; initial wait remains above target. No random-stream or art changes were made.

**Next:** review the poor-placement and turning trials; retain only verified improvements. The primary campaign gate compares reference against a deliberately poor legal setup, while empty/fan-only are diagnostic controls. Freeze useful reference/poor setups and thresholds for both houses within the time limit. Then freeze thresholds/content for paired30-seed tuning and disjoint30-seed validation, complete sustained production performance/memory/cancellation/input/hidden-tab checks and available platform coverage. Preserve sensory-only neural input, native contact and recorded replay; no invisible food enlargement or target steering. Actual Safari and the older microscopic contact-interpolation limitation remain explicit open coverage/issues.
