...
**Current pickup:** complete the user-requested global3× scent-emission comparison before choosing a production policy. Banana-only amplification was an isolation experiment, not the intended final rule. The global probe in `/tmp/fly-scent-strength/crates/sim/examples/global_scent_probe.rs` scales authored attractive/repellent sources AND every fixed/placed catalog odor source. Light, exit cue, airflow and doorway-only suction remain unchanged. Output is `/tmp/fly-global-scent-results`; binary/saturating seed0 jobs are running, then both seed1 jobs remain. Each job contains global3× empty and one-banana arms,16 flies,6000ticks. Reuse completed normal-emission controls from [scent-strength evidence](assets/evidence/33/scent-strength/README.md). Confirm source scaling validation in `/tmp/fly-global-validation.log` before accepting results. Root owns this probe; the Claude run is terminal.

The banana-only matrix is complete: normal→threefold banana emission improved escapes in both paired seeds under both encodings. Binary totals5 empty /3 normal /9 stronger; concentration totals3/6/11, each out of32. The candidate uses c/(c+0.55), changes both attractive and repellent input and lowers finite-concentration drive; cross-encoding outcomes are not a pure sensitivity comparison. Production encoding/emission remain unchanged.

The clustering comparison is secondary to the new global request. `/tmp/fly-cluster-comparison` has completed cluster seed0/1 (escapes1/5) and spread seed0 (escape1); spread seed1 remains. These runs use16 flies and original binary/doorway-only behavior, so do not compare counts with earlier20-fly cohorts or the integrated room-pull build. Preserve star thresholds1/8/15 and timer. Production's gentle room pull remains provisional; no-object browser integration completed but did not establish better balance.

**Latest placement evidence:** [five bananas](assets/evidence/33/five-bananas/README.md) produced final-room entrants7/14/12 and escapes2/8/6, compared with empty8/8/8 entrants and5/5/4 escapes. This suggests aggregate recruitment, with modest escape improvement and substantial seed variability. [One-banana comparisons](assets/evidence/15/food-after-contact/README.md) and [single-fan comparisons](assets/evidence/15/single-fan-control/README.md) remain controls, not evidence of reliable food-placement advantage. The [isolated scent comparison](assets/evidence/33/banana-direction/README.md) establishes a nearby directional bias, not useful distant recruitment. Both levels retain the user-required1/8/15 star thresholds. Level1 has a ten-minute horizon; release buffering/performance remains open.

**Independent integration:** [gentle final-room pull](assets/evidence/15/final-room-pull/README.md) is verified in the root build with55 Rust body/attempt tests, WASM, TypeScript and web builds. The no-object browser replay completed with3 escapes and zero underruns; this is integration evidence, not improved balance. Keep it out of the ongoing scent comparison. [Foreground-only wall fading](assets/evidence/31/foreground-walls/README.md) preserves opaque back walls; remaining translucent bands, small selected flies and terminal trails are deferred under the mechanics-first priority.

**Priority order:** plausible sensory wiring and meaningful aggregate placement control, then two playable levels and star thresholds, then final performance/platform gates. Preserve variable responses and honest model limitations; do not tune neural signs to manufacture predetermined behavior. Exit suction stays identical within each comparison and restricted to the exit room; do not mix the pending gentle room pull with older doorway-only baselines. Test without fans first; the single fan cannot substitute for useful object-placement mechanics. Holdout remains untouched. Feeding and visual polish remain deferred.

**Evidence ledger:**
- [Biological grounding](assets/evidence/32/food-strategy/biology.md), [readout correction](assets/evidence/32/readout-anatomy/README.md), [DA2 pulse](assets/evidence/33/da2-pulse/README.md), [input-rate fit](assets/evidence/33/geosmin-encoding/README.md): source annotations and primary studies distinguish supported anatomy from unvalidated encoding. Production odor inputs still bypass receptors; the DA2 graph and inputs are scratch only.
- [Food strategy](assets/evidence/32/food-strategy/README.md): matched food layouts change residence and escape in the previous model, but longer timing does not establish route-food benefit. This is not biological validation or accepted campaign balance.
...
- [ ] [33 — Biologically motivated sensory inputs](slices/33-biological-sensory-grounding.md) — document assumptions and support exploratory play; reliable repulsion and complete circuit explanations are not required.
