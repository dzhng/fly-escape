...
Continue [20 — furnishing and food contact](slices/20-furnished-contact.md), starting with supported movement and recorded support identity. The query kernel, fixed orientation binding and physical floor height/replay are verified. The food-geometry/identity pass replaces edible circles with baked Blender triangles and one prepared world shared by all flies. Its verification and remaining limits are recorded under [food geometry evidence](assets/evidence/20/food-geometry/README.md).

First resolve the [native tangent-motion reproduction](assets/evidence/20/support-motion/README.md): the candidate can report false horizontal floor obstructions. Then resolve native-hull acquisition, walking, feeding, loss, departure and revisit, followed by curved replay and selection-ring conformance. Food proximity still uses the provisional body radius; it is not apple landing/walking acceptance. Floor meal and neural-ablation checks pass, but curved food movement and20-brain integration remain open. Do not rebuild the query owner, restore renderer-only height or use the test ball as the fly body.
