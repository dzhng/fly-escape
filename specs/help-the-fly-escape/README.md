...
Continue [20 — furnishing and food contact](slices/20-furnished-contact.md), starting with physical acquisition, supported movement and recorded support identity. The query kernel, fixed orientation binding and physical floor height/replay are verified. The food-geometry/identity pass replaces edible circles with baked Blender triangles and one prepared world shared by all flies. Its verification and remaining limits are recorded under [food geometry evidence](assets/evidence/20/food-geometry/README.md).

The [support/orientation record boundary](assets/evidence/20/support-record/README.md) is integrated through actual WASM transport. The core currently records upright orientation and null food support; this is not physical apple acquisition. The tangent-query regression is resolved. The [shared support sampler](assets/evidence/20/support-sampling/README.md) now provides a native root separately from its contact witness, with matching native/WASM evidence. Next connect it to physical acquisition, walking, feeding, loss, departure and revisit, followed by curved replay and selection-ring conformance. Valid sampled poses do not prove the path between them; per-render-frame sampling is not an accepted performance design. Food proximity still uses the provisional body radius; it is not apple landing/walking acceptance. Floor meal and neural-ablation checks pass, but curved food movement and20-brain integration remain open. Do not rebuild the query owner, restore renderer-only height or use the test ball as the fly body.
