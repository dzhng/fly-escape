...
Continue [20 — furnishing and food contact](slices/20-furnished-contact.md) by retaining the [verified original-plane boundary](assets/evidence/20/support-path-feasibility/native-envelope/plane-inventory/README.md) in the existing offline candidate asset. Original-plane fixed paths pass; an independent exact-integer check confirms the stored planes' boundary connections. Reconstructed QuickHull face/edge topology fails, so do not infer this inventory again from nearly coplanar points. The shape remains a candidate, not an adopted body. Preserve containment, visible contact, world scale and numerical tolerance. Then continue [continuous path work](assets/evidence/20/support-path-feasibility/README.md): global competing contacts and general translating rotation remain unaccepted.
