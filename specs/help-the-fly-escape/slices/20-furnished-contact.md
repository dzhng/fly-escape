...
Status: **in progress**. Native contact, body integration, recorded curved motion and a controlled actual-graph banana meal are implemented. Remaining gameplay gates are robust timed-round contact and sustained twenty-fly cost. Feeding/revisit is deferred by the user. Cosmetic contact refinement is deferred by the user’s MVP visual acceptance; functional transition/replay checks remain required. Dependencies: 19,25,26 and the physical-adapter verdict.
...
2. **Feeding revisit deferred; preserve recorded contact.** The user removed eating/revisit as an MVP requirement. Retain physical landing, walking, support acquisition/loss, departure, terminal freeze and replay correctness, including pause and reverse seek on the authoritative sampler. Legitimate feeding revisits and matched replenishment controls are deferred. Keep twenty independent streams.
