...
A visually millimetre-sized fly must not retain an invisible eighty-millimetre collision body or antenna samples hundreds of millimetres apart. The neutral room in 19 exposed this mismatch. Reconcile physical dimensions through the existing BodyConfig/FieldConfig owners before authoring final food/contact geometry. Do not add a renderer-only physics multiplier or change the connectome.

First publish measured model body/antenna landmarks and the conservative collision envelope over relevant animation poses, in the declared metre world. Keep anatomical sampling locations distinct from the collision envelope: wing tips are not antennae. Record velocity, tool/contact footprints and field-grid units alongside the proposed dimensions. Existing values are model choices, not biological measurements.

Then run the existing mirrored sensory/body fixtures at those physical dimensions with the same real graph and frozen neural equations. Compare left/right sampled concentrations, relative contrast, injected sensory currents and downstream neural/body response; include neutral, mirrored and pathway-silenced controls. The current five-percent cue contrast cutoff is frozen for this reproduction. If the anatomical sampling span makes it fail, retain the failed evidence and specify a separate measured sensory-adapter correction before changing that cutoff. Do not lower a detector threshold quietly or substitute direct heading control.

## Review surface and gates

A dimension sheet and bounded native/WASM sensory probe establish one consistent body/environment scale. A diagnostic browser overlay may show the true body envelope and antenna sample points against the rendered fly. No long campaign batches run until this seam and food contact are settled.
