...
## Prepared closed window

The [window preparation](../assets/evidence/21/window-prepared/README.md) preserves
the existing room-scale envelope and provides closed panes, backed sash reveals
and an asymmetric handle. Neutral authoring and export checks pass; the model is now mounted through the production room-detail owner. Final glass/light treatment and composed occlusion remain open. It does not create a new physical opening.

## Household sconce

Prepare a [neutral wall sconce](../../../assets/house/sconce/README.md), separate from the floor-source gameplay lamp. Assume a compact 22 × 32 × 16 cm household fixture with bottom-centred local origin and flat rear mounting plane. A tapered shade, connected curved arm and backplate should read as a wall fixture without emission or colour. Shared export/stage helpers own those seams. Native roundtrip, closed-part topology and full/crop critique gate this shape; runtime mounting and decorative point lighting are now integrated through the room-detail owner. This does not add a gameplay light cue or separate physical obstacle.

## Prepared-shape browser inspection

Extend the existing proportions workbench with native replacement inspection for window, sconce and plant. Its explicit appearance-only slot belongs to WorldView resource disposal but adds no core occupancy, wall opening, emission or catalogue entry. The static GLB validator is shared with catalogue-backed house loading; native bounds are checked without scaling. Asset framing uses the existing camera owner and leaves recorded fly poses untouched. A named lower mounting view is diagnostic-only; Follow and Overview restore the ordinary RTS elevation. Diagnostic mounting is a review assumption, not final campaign placement. Remove this dedicated inspection composition when accepted physical placement contracts can drive the same asset review. [Integrated browser evidence](../assets/evidence/21/browser-shapes/README.md) verifies replacement failure/staleness, resource retirement, recorded replay and full/crop inspection. Catalogue furnishings are physically adopted. Closed windows, sconces and the wall-mounted plant are installed as decorative room details, not new physical obstacles. The separate floor-plant foliage/top-contact study remains unadopted; final materials and composed visibility remain open.

The [domestic furnishing integration](../assets/evidence/22/domestic-furnishings/README.md) installs recognizable kitchen, study and bedroom furniture in both authored houses at native dimensions. Closed floor bases preserve truthful blocked footprints under the current geometry owner. Final surface detail and exit-window identity remain open.

The [web fan evidence](../assets/evidence/21/web-attachment/README.md) accepts a bounded asymmetric composition and right-base mounting improvement. Room-scale silk readability, visibly convincing upper/floor attachments and rigid spider legs remain open; native contact still follows the exact visible strands.
