...
The [sustained retry harness](../assets/evidence/17/full-retry-harness/README.md) now runs complete actual campaign attempts at 1× and can replay captured seeds through the normal setup UI. It preserves cancellation as the default, saves failure evidence, and rejects vacuous post-warm-up comparisons. Its random second-house run exposed a new supported-motion refinement failure; the release gate remains red until that exact case and affected gates pass.
