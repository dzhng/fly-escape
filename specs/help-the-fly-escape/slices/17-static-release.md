...
The [timed integration](../assets/evidence/17/timed-rounds/README.md) completed five-minute attempts in both houses without starvation or replay underruns. Those historical runs took84–98seconds to start; the newer [paced second-house production run](../assets/evidence/16/timed-pacing/README.md) measured36.901seconds. Startup remains above target; raising observation timeouts measures it without accepting it. The production work is bounded by20flies and3000ticks and advances normally, so retry/backoff changes do not address the cause.

The [named-WASM and native diagnosis](../assets/evidence/17/neural-cost/README.md) identifies neural computation as the dominant cost. The next performance pass targets it while preserving exact arithmetic order, stochastic streams and actual-graph behavior. Verify an optimization against baseline neural values and seeded attempt outcomes before measuring rebuilt browser startup. Avoid conflating fewer active flies, easier geometry or altered neural rules with faster computation. Full campaign balance and final resource/platform gates still follow.

[Between-tick task yielding](../assets/evidence/17/task-yield/README.md) removes measured nested-timer delay without changing credit or buffer safety. The matched integrated full attempt starts below the startup threshold and preserves recorded-stream checksums; sustained final-build and platform acceptance remain separate.
