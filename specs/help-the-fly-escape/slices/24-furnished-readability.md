...
The slice19 3 mm fly/80 mm apple diagnostic places the macro Follow camera inside the nearby apple, hiding the selected fly even after correcting near-plane clipping; clear-floor Follow and closer zoom isolate successful scale framing. Preserve that fruit-adjacent pose as an occlusion regression. Food support and authored food volumes must participate in the same selected-subject occlusion owner as furnished room geometry. A frustum-visible subject is not proof of an unobscured subject. Do not fix this by enlarging the fly, flattening real fruit or silently moving the camera/food in the regression fixture.

At the declared millimetre fly scale, the current physical ring is subpixel at room Overview. Establish a readable minimum screen-space selection cue independently of the fly's physical size; preserve the true model scale and test the same room-wide view.
