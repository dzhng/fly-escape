...
The spec delegates internal file/report layout and numerical tie handling. The original hard-sector registration is retained with its historical evidence. The overlapping projection follows the separately frozen slice-4 protocol.
...
- **Allow rounding error at the unit row bound — high confidence.** A generated cell can sum to 1.0000000000000002 because binary arithmetic cannot represent every weight exactly. Rejecting it would prevent a valid map from loading. The loader permits a trillionth of rounding error, and the current calculation caps the result at the configured gain. The protocol defines the bound but leaves floating-point tolerance implicit; this keeps all generated cells usable without allowing a meaningful dose increase.

- **Reuse the existing workload benchmark with supplied content — high confidence.** To measure the visual adapter, the benchmark accepts a level and tuning file while keeping its original default. Both light-on and light-off runs therefore use the same actual house and body settings, and the benchmark still rejects stopped flies. The plan required a throughput comparison without choosing a harness; extending the existing measurement owner avoids a second timing loop. This adds one optional benchmark argument, not a gameplay option.
