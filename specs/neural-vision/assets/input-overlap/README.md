...
These maps implement the [bounded overlapping-input revision](../../slices/04-overlapping-inputs.md). They keep the original eligible neurons, source ranges, modeled preferred angles and retained graph paths. The runtime representation is one unique entry per neuron with eight nonnegative weights; the [hard-sector maps](../input-map/README.md) remain unchanged historical evidence, not an alternative runtime format.
