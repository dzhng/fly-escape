...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let path = args
        .get(1)
        .ok_or("Pass graph directory, fly count, measured ticks, output JSON, optional content JSON")?;
    ...
    let mut level = swarm_lab::level(count)?;
    ...
    if let Some(content_path) = args.get(5) {
        let content: serde_json::Value = serde_json::from_slice(&std::fs::read(content_path)?)?;
        level = serde_json::from_value(content["level"].clone())?;
        tuning = serde_json::from_value(content["tuning"].clone())?;
    }
