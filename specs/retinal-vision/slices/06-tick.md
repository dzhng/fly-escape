...
**Status:** passed. The native transaction, production capture/commit worker, exact recorded-input WASM reconsumption, cancellation at scene/map/capture boundaries, failure recovery and watchdog are integrated. See [native seam](../assets/06/native-seam.md) and [browser evidence](../assets/06/browser-integration.md). Final cutover must rerun these gates against the final manifest.
