...
**Status:** native preparatory seam implemented and verified; browser integration remains open. The [native seam evidence](../assets/06/native-seam.md) records the tested transaction, concrete sparse RGB adapter and explicit record-packing block. Default gameplay is unchanged; real acquisition, browser failure handling and numerical WASM execution still need their integration gates.
