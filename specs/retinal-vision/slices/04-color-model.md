...
**Status:** offline source audit/model freeze complete; see [the frozen model and rationale](../assets/04/README.md) and [verification](../assets/04/verification.md). The source-supported approximation uses Tm2 brightness plus whole-population Tm20 blue. Tm5 candidates lack finite source columns and are excluded. Coefficients are explicitly unmeasured, not physiological fits. Slice 05 still owns the combined weighted-current budget and matched spatial support; slice 09 owns downstream chromatic proof with the frozen equal-dose controls. Independent code review and all 32 connectome tests pass. Final unprimed atlas review passes; human review is coordinated by the integrating agent.
