...
ROOT = HERE.parents[3]
OLD = ROOT / 'specs/retinal-vision/assets/08'
MAP = ROOT / 'specs/retinal-vision/assets/05/retinal-map.json'
...
def prepare():
    ...
    proposal=dict(status='INPUT-ONLY PROPOSAL; not preregistered or authorized for Brain execution', runAuthorized=False,
        ...
        panels=panels, sourceHashes={'prepare.py':sha(Path(__file__).read_bytes()),'native/src/main.rs':sha((HERE/'native/src/main.rs').read_bytes()),
            'original08Freeze':sha((OLD/'confirmation/freeze.json').read_bytes()),'original09Freeze':sha((ROOT/'specs/retinal-vision/assets/09/confirmation/freeze.json').read_bytes())})
