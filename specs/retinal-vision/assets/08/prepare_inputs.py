...
def prepare(root, optics, benchmark, output, occlusion=None):
    map_dir = root / "specs/retinal-vision/assets/05"
    ...
    for slice_id, pack in packs.items():
        ...
        pack["analysisSha256"] = digest((root / "specs/retinal-vision/assets/08/analyze.py").read_bytes())
        pack["statisticsOwnerSha256"] = digest((root / "scripts/connectome/analyze_neural_vision.py").read_bytes())
        pack["preregistrationSha256"] = digest((root / f"specs/retinal-vision/assets/{slice_id}/preregistration.md").read_bytes())
...
if __name__ == "__main__":
    ...
    prepare(root, args.optics, root / "specs/retinal-vision/assets/02/higher-resolution/benchmark.json", args.output, args.occlusion)
