...
## One-fly diagnostics and final cutover — sound

- **Use the campaign producer for the one-fly workbench.** A short fixed walking start supplies real native body poses and ordinary campaign cues. History controls read the archive; they never recapture or advance the brain. Exporting the complete start/configuration/frames allows the native binary to reconsume exactly the same input. High confidence from full-frame native/browser correspondence.
- **Expose bounded stage metadata without copying observations.** The existing worker progress message can identify its pending tick, and a generation-gated observer displays capture versus computation. No new tick owner or RGB stream is introduced. High confidence from cancellation/stale-generation and slider ownership checks.
- **Keep scalar field diagnostics separate from neural vision.** Brightness/shade fields still serve existing non-neural field reports; every visual neural cue requires paired RGB acquisition. Legacy directional metadata is rejected instead of being retained as opaque provenance. High confidence: the old sampler, adapter, types and placeholder record fields are deleted.
- **Keep roster interaction native.** The requested horizontal strip retains the existing buttons, labels, tab order and selection owner. A small scroll cue communicates overflow. Unselected cards can be partly outside the viewport; selection always brings the complete chosen card into view. High confidence from all-sixteen keyboard and external-selection checks at both widths.
