...
This proposal is not itself a runnable experiment manifest and `runAuthorized` remains false. The separate version-2 runner/analyzer contract uses the accepted seeds and input design only through the reviewed protocol and post-cutover preparation path. Preparation and freeze must bind the final metadata/source identities before any run. Old frozen directories are never reused; their exact source remains available in the original commits.
