...
The [time-resolved readout observation](time-resolved/report.md) reproduces every historical aggregate and the exact voltage decoder while inspecting all turning members across an internal seed split. Stimulus-related readout activity is present, but directional-pattern replication remains weak; neither decoder cancellation nor absence is established. This does not change the negative approach conclusion or authorize neural changes.

The [time-resolved readout observation](time-resolved/report.md) reconstructs the current motor decoder exactly across all 12,000 frames and reproduces every prior aggregate. A fixed internal seed split does not establish reliable mirrored directional activity hidden by the population average. It supports neither a decoder change nor proof that directional information is absent.
