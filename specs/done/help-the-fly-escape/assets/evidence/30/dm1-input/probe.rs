...
fn main()->Result<(),Box<dyn std::error::Error>>{
   ...
   for seed in 0..30{
    ...
    for active in [false,true]{
     ...
     for tick in 0..100{
      ...
      for g in &frame.neural.groups { if ["odorInhL","odorInhR","dm1PNL","dm1PNR"].contains(&g.id.as_str()) { *activity.entry(format!("{}MeanVoltage",g.id)).or_default() += g.mean_voltage/100.; *activity.entry(format!("{}SpikeFraction",g.id)).or_default() += g.spike_fraction/100.; } }
 ...
 std::fs::write("/tmp/fly-dm1-probe/evidence.json",serde_json::to_string_pretty(&output)?)?;Ok(())
}
