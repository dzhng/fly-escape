...
//! attempt is constructed. Optional `speedScale`/`turnScale`/`odorGain` are scratch candidates;
...
struct Plan {
    ...
    #[serde(default = "one")]
    speed_scale: f64,
    /// Scratch turning-response candidate only; 1.0 leaves the shipped turnGain untouched.
    #[serde(default = "one")]
    turn_scale: f64,
    /// Scratch odor-cue candidate only; `null` leaves the shipped AttemptTuning.cues gains
    /// untouched. When set, it replaces the gain of the excitatoryOdor and inhibitoryOdor
    /// cue entries that the level already authors. Taste and vision are never touched.
    #[serde(default)]
    odor_gain: Option<f64>,
}
fn one() -> f64 {
    1.
}
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for plan in &plans {
        ...
        if !(plan.turn_scale.is_finite() && plan.turn_scale > 0.) {
            return Err(format!("plan {} has a non-positive turn scale", plan.label).into());
        }
        if let Some(g) = plan.odor_gain {
            if !(g.is_finite() && g > 0.) {
                return Err(format!("plan {} has a non-positive odor gain", plan.label).into());
            }
            if !tuning
                .cues
                .iter()
                .any(|c| matches!(c.pathway, CuePathway::ExcitatoryOdor | CuePathway::InhibitoryOdor))
            {
                return Err(format!("plan {} sets odorGain but the level authors no odor cue", plan.label).into());
            }
        }
        ...
        eprintln!(
            "plan {} accepted by resolve_placements ({} placements, speedScale {}, turnScale {}, odorGain {:?})",
            plan.label,
            plan.placements.len(),
            plan.speed_scale,
            plan.turn_scale,
    ...
    for &seed in &seeds {
        for plan in &plans {
            ...
            level.body_config.turn_gain *= plan.turn_scale;
            let mut tuning = tuning.clone();
            if let Some(g) = plan.odor_gain {
                for cue in &mut tuning.cues {
                    if matches!(
                        cue.pathway,
                        CuePathway::ExcitatoryOdor | CuePathway::InhibitoryOdor
                    ) {
                        cue.gain = g;
                    }
                }
            ...
            let report = json!({
                ...
                "turnScale":plan.turn_scale,
                "odorGain":plan.odor_gain,
                "cues":tuning.cues,"tasteGain":tuning.taste_gain,
                "tuningHash":spec.tuning_hash.clone(),
                "walkSpeed":level.body_config.walk_speed,
                "flightSpeed":level.body_config.flight_speed,
                "turnGain":level.body_config.turn_gain,
