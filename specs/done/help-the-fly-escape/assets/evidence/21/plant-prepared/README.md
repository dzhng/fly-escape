...
The three current full views opened in one Preview window at 09:53:39UTC on 2026-09-07. The window closed at 09:58:57UTC after the non-blocking feedback period without feedback; this is not user approval.

The shared neutral-stage extraction preserves geometry and framing: [verification](shared-stage-verification.json) compares old/new Blender stage settings for cabinet, sofa and plant, proves each GLB BIN chunk unchanged, and records a matched plant three-quarter render pair. The authoring-stage helper owns common light/world/render settings; each asset still owns camera positions, target, scale and ground extent. Saved asset binaries remain unchanged. This is cleanup verification, not a new visual acceptance gate.
