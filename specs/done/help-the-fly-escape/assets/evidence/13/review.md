...
The panel is now merged with absolute animation sampling, the current authored fly and the shared selection action. All15 client tests (132 assertions), renderer checks and typecheck pass. The merged science browser check passes all20 cards/40 plots, shared timestamps, last-card selection and desktop/narrow explanations; the existing full playback-control check also passes. Captures and exact results are under `integrated/` and `integrated-playback/`.

The tooltip now defines neuron, synapse, membrane voltage, spike and refractory period in place, so its explanation does not require prior neuroscience knowledge. It describes neuronal signals and model assumptions rather than placement tactics. The additional text remains inside both tested viewport bounds.

Final image-only adversarial check: the long explanation could overwhelm or clip in a narrow window; actual captures keep its close action and text visible with bounded scrolling. Dense same-column arrows could obscure the short group labels; the labels remain outside the connection gutter, with the disclosed compact-network limitation unchanged. Existing independent critiques covered the layout; a further fresh-agent request for the final wording-only capture was unavailable under the thread limit. Root inspected the new full/crop captures directly. No new scientific interpretation or signal encoding was introduced by integration.

Current20-fly memory accounting with the real model, animation and science cards totals257,700,716 bytes (about246 MiB), below the512 MiB owned-memory target. [Integrated memory](integrated-memory/browser-memory.json) includes measured main/Worker heaps, full-archive allowance, baseline full-run WASM high water and renderer GPU estimate. It deliberately permits double counting; it is not browser-process RSS and excludes driver/compositor overhead. The simulation build identity matches the baseline.
