...
async function replace(source: Promise<ArrayBuffer>) {
  ...
  try {
    ...
    if (ticket === generation) status.textContent = `Could not load model. ${app.dataset.resources ? "The previous fly is still displayed." : "Choose a valid GLB to continue."} ${error instanceof Error ? error.message : String(error)}`;
  }
}
